...
import { Users, Briefcase, Fuel, Check, ArrowRight, Sparkles, CarFront } from "lucide-react";
...
export function Fleet() {
  ...
  // GSAP Animation for switching cars
  useGSAP(() => {
    const ctx = gsap.context(() => {
      // 1. Image Animation: Slide in from right + Fade
      gsap.fromTo(
        ".car-image",
      ...
      // 2. Text Content: Staggered Fade Up
      gsap.fromTo(
        ".animate-item",
        { y: 20, opacity: 0 },
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 relative z-10">
        ...
        <div className="mb-12 md:mb-20 text-center max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 text-sm md:text-base leading-relaxed">
            Choose the perfect ride for your journey. From economical daily commutes to luxury outstation travel, we have it all.
          </p>
        </div>

        {/* --- STICKY TABS --- */}
        <div className="sticky top-20 md:top-6 z-40 mb-12 flex justify-center">
          <div className="flex p-1.5 gap-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
            {fleetData.map((car, index) => (
              <button
                ...
                className={`
                  relative px-6 py-2.5 md:px-8 md:py-3 rounded-full text-sm font-semibold transition-all duration-300
        ...
        {/* --- MAIN CONTENT AREA --- */}
        <div ref={contentRef} className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-16 items-center">
            ...
            <div className="space-y-8">
              ...
              <div className="animate-item grid grid-cols-3 gap-3 py-6 border-y border-white/10">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white/5 border border-white/5 text-center transition-colors hover:bg-white/10 group">
                    <spec.icon className="h-5 w-5 text-gray-500 group-hover:text-purple-400 transition-colors mb-2" />
